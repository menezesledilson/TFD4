...
class pacienteDAO
{
    ...
    {
        try {
            $stmt = $this->conexao->getConexao()->prepare("UPDATE pacientes AS p INNER JOIN situacoes AS s ON p.id_situacao = s.id INNER JOIN unidades AS u ON p.id_unidade_usf = u.id SET p.nome = ?, p.rg = ?, p.cpf = ?, p.cns = ?, p.celular = ?, p.endereco = ?, p.numero = ?, p.bairro = ?, p.cidade = ?, p.cep = ?, p.id_situacao = ?, p.id_unidade_usf = ? WHERE p.id = ?");
            if (!$stmt) {
                throw new Exception("Erro ao preparar a consulta: " . $this->conexao->getConexao()->error);
            }
