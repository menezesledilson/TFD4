...
class AcompanhanteDAO
{
    ...
    public function postAcompanhante($nome, $rg, $cpf, $telefone, $endereco, $numero, $bairro, $cidade, $cep, $id_situacao, $created, $modified)
    {
        try {
            $stmt = $this->conexao->getConexao()->prepare("INSERT INTO acompanhantes (`nome`, `rg`, `cpf`, `celular`, `endereco`, `numero`, `bairro`, `cidade`, `cep`, `id_situacao`,`created`,`modified`) VALUES (?,?,?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
            $stmt->bind_param("ssssssssssss", $nome, $rg, $cpf, $telefone, $endereco, $numero, $bairro, $cidade, $cep, $id_situacao, $created, $modified);
            if ($stmt->execute()) {
                return true;
            } else {
                return false;
    ...
    public function putAcompanhante($nome, $rg, $cpf, $celular, $endereco, $numero, $bairro, $cidade, $cep, $id_situacao, $modified, $id)
    {
        try {
            $stmt = $this->conexao->getConexao()->prepare("UPDATE acompanhantes AS a 
                                                       INNER JOIN situacoes AS s ON a.id_situacao = s.id 
                                                       SET a.nome=?, a.rg=?, a.cpf=?, a.celular=?, a.endereco=?, a.numero=?, a.bairro=?, a.cidade=?, a.cep=?, a.id_situacao=?, a.modified=? 
                                                       WHERE a.id=?");
            if (!$stmt) {
                throw new Exception("Erro ao preparar a consulta: " . $this->conexao->getConexao()->error);
            }
            $stmt->bind_param("sssssssssssi", $nome, $rg, $cpf, $celular, $endereco, $numero, $bairro, $cidade, $cep, $id_situacao, $modified, $id);
            if ($stmt->execute()) {
                return true;
            } else {
                return false;
